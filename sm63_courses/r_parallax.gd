...
@export var game_resolution: Vector2 = Vector2(1920, 1080) / 3.0
...
func _process(delta):
	...
	if not Engine.is_editor_hint():
		...
		var camera_pos = (canvas_pos - screen_size / 2.0) * canvas_scale_inv # matches camrea node position
		...
		transform = transform.translated(-camera_pos)
